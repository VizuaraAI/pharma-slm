...
{\sffamily\bfseries\fontsize{30}{34}\selectfont\color{ink} Building a Pharma LLM\\[2mm] from Scratch}\\[6mm]
{\sffamily\Large\color{slate} How an enterprise trains its own language model ---\\ every step, every experiment, every GPU, every dollar.}\\[14mm]
...
\input{part_foundations}
\input{part_building}
\input{part_research}
\input{part_further}
\input{part_engineering}
\input{part_production}
\input{appendix}
