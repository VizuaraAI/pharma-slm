...
\fig{figures/diag_datapipeline.png}{0.98}{The data pipeline. Open sources are tokenized in parallel across
$\sim$24 cloud workers into per-source binary token files; a weighted mixing loader then samples them at batch
time, so the general/domain ratio is a runtime knob rather than a re-tokenization job.}
...
\fig{figures/diag_tokenizer.png}{0.98}{Our byte-level BPE in action. Text is pre-tokenized into bytes, then
32{,}000 learned merges reassemble it into tokens. Because the vocabulary was trained on pharmaceutical text,
domain words stay whole (1--2 tokens) where a general tokenizer would shatter them (5--6 tokens) --- a
fertility of 1.57 on dense pharma text.}
...
\fig{figures/diag_architecture.png}{0.46}{The exact architecture we built. Token IDs are embedded
(vocab 32{,}000 $\times$ dim 2{,}048 for the 1.3B; dim 1{,}024 for the 350M), passed through 24 decoder layers,
a final RMSNorm, and a weight-tied linear head to produce logits over the 32{,}000-token vocabulary.}
...
\fig{figures/diag_block.png}{0.96}{One decoder layer, the pre-norm design: each sublayer normalizes its
input, transforms it, and adds the result back to a residual skip-connection ---
$x \leftarrow x + \text{Attn}(\text{RMSNorm}(x))$, then $x \leftarrow x + \text{SwiGLU}(\text{RMSNorm}(x))$.
The skip-connections are what let gradients flow cleanly through all 24 layers.}

\fig{figures/diag_attention.png}{0.96}{Inside the attention sublayer. The input is projected to queries, keys,
and values; rotary position embeddings (RoPE) are applied to $Q$ and $K$; scaled dot-product attention runs
with a causal mask (each token sees only the past) across 16 heads of dimension 128; the heads are
concatenated and projected out.}

\fig{figures/diag_swiglu.png}{0.96}{Inside the feed-forward sublayer. SwiGLU is a \emph{gated} MLP: one linear
branch passes through a SiLU activation and gates a second linear branch element-wise, before a final
down-projection --- $\text{SwiGLU}(x) = W_2\,(\text{SiLU}(W_1 x) \odot W_3 x)$, hidden width 5{,}632.}
